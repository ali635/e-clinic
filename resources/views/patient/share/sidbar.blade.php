    ...
    <section class="profile_layout bg-gray-50">
        <div class="flex gap-4 relative">
            ...
            <aside id="sidebarMenu" class="sidebarMenu bg-white shadow-xl rounded-r-2xl border-r border-gray-200">
                ...
                <div class="p-6 border-b border-gray-200 bg-gradient-to-br from-primary/5 to-transparent">
                    <div class="flex items-center gap-4">
                        <div class="relative flex-shrink-0">
                            <img onerror="if(!this.dataset.fallback){this.dataset.fallback=true;this.src='{{ asset(setting('site_logo')) }}';}else{this.onerror=null;}"
                                src="{{ asset(auth('patient')->user()->img_profile ?? '') }}"
                        ...
                        <div class="min-w-0 flex-1">
                            <h3 class="text-sm font-bold text-gray-900 truncate">
                                {{ auth('patient')->user()->name ?? 'Guest User' }}</h3>
                            <p class="text-xs text-gray-600 font-medium truncate">{{ auth('patient')->user()->email ?? '' }}
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <ul class="space-y-2">
                        <li>
                            <a href="{{ route('patient.profile.data') }}"
                                ...
                                @if (request()->routeIs('patient.profile.*')) bg-gradient-to-r from-primary/10 to-transparent text-primary border-l-4 border-primary 
                                @else 
                                    text-gray-700 hover:bg-gray-100 hover:translate-x-1 @endif">
                                <svg class="w-5 h-5 mr-3 flex-shrink-0" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <li>
                            <a href="{{ route('patient.dashboard') }}"
                                class="sidebarMenu_item flex items-center px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200
                                @if (Route::currentRouteName() == 'patient.dashboard') bg-gradient-to-r from-primary/10 to-transparent text-primary border-l-4 border-primary 
                                @else 
                                    text-gray-700 hover:bg-gray-100 hover:translate-x-1 @endif">
                                <svg class="w-5 h-5 mr-3 flex-shrink-0" xmlns="http://www.w3.org/2000/svg"
                                    viewBox="0 0 24 24" fill="currentColor">
                        ...
                        <li>
                            <a href="{{ route('patient.visits') }}"
                                class="sidebarMenu_item flex items-center px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200
                                @if (Route::currentRouteName() == 'patient.visits' || Route::currentRouteName() == 'patient.visits.show') bg-gradient-to-r from-primary/10 to-transparent text-primary border-l-4 border-primary 
                                @else 
                                    text-gray-700 hover:bg-gray-100 hover:translate-x-1 @endif">
                                <svg class="w-5 h-5 mr-3 flex-shrink-0" xmlns="http://www.w3.org/2000/svg"
                                    viewBox="0 0 24 24" fill="currentColor">
                        ...
                        <li>
                            <a href="{{ route('patient.history') }}"
                                class="sidebarMenu_item flex items-center px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200
                                @if (Route::currentRouteName() == 'patient.history') bg-gradient-to-r from-primary/10 to-transparent text-primary border-l-4 border-primary 
                                @else 
                                    text-gray-700 hover:bg-gray-100 hover:translate-x-1 @endif">
                                <svg class="w-5 h-5 mr-3 flex-shrink-0" xmlns="http://www.w3.org/2000/svg"
                                    viewBox="0 0 24 24" fill="currentColor">
                        ...
                        <li>
                            <a href="{{ route('patient.feedback') }}"
                                class="sidebarMenu_item flex items-center px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200
                                @if (Route::currentRouteName() == 'patient.feedback') bg-gradient-to-r from-primary/10 to-transparent text-primary border-l-4 border-primary 
                                @else 
                                    text-gray-700 hover:bg-gray-100 hover:translate-x-1 @endif">
                                <svg class="w-5 h-5 mr-3 flex-shrink-0" xmlns="http://www.w3.org/2000/svg"
                                    viewBox="0 0 24 24" fill="currentColor">
                ...
                <button
                    class="sidebarToggleBtn bg-primary text-white shadow-lg hover:bg-primary/90 transition-colors duration-200"
