    ...
    <main class="flex-1 px-4 sm:px-6 lg:px-8 py-6 md:py-8">
                        ...
                        <div>
                            ...
                            @if ($visit->is_arrival)
                                <p class="text-green-600 font-semibold flex items-center gap-2">
                                    ...
                                    {{ __('Yes') }}
                                    <span class="text-gray-600 text-sm font-normal">({{ __('Arrival Time') }}: {{ $visit->arrival_time }})</span>
                                </p>
                            @else
                ...
                <div class="space-y-4">
                    <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                        ...
                        <div>
                            ...
                            <p class="text-gray-900 font-medium">{{ \Carbon\Carbon::parse($visit->arrival_time)->format('Y-m-d h:i A') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                @else
                                    <span class="text-gray-900 font-medium">${{ number_format($visit->total_price, 0) }}</span>
                                @endif
            ...
            <section class="bg-white rounded-2xl shadow-lg p-6 sm:p-8 mb-8">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 mb-6">
                    @foreach ($visit->lab_tests as $lab_img)
                        <a href="{{ asset('storage/' . $lab_img) }}" 
                           data-lightbox="lab-tests" 
                           class="group relative block rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 transform hover:scale-105">
                            <div class="aspect-w-1 aspect-h-1 w-full h-32 bg-gray-100">
                                <img src="{{ asset('storage/' . $lab_img) }}" 
                                     alt="{{ $visit->service->name }}"
            ...
            <section class="bg-white rounded-2xl shadow-lg p-6 sm:p-8 mb-8">
                ...
                <div class="flex flex-wrap gap-3">
                    @foreach ($visit->medicines as $medicine)
                        <span class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-primary/10 to-primary/5 text-primary font-medium rounded-full shadow-sm hover:shadow-md transition-all duration-200">
            ...
            <section class="bg-white rounded-2xl shadow-lg p-6 sm:p-8 mb-8">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    @if ($visit->doctor_description)
                        <div class="bg-gradient-to-br from-primary/5 to-transparent rounded-xl p-5 border border-primary/20">
                    ...
                    @if ($visit->chief_complaint)
                        <div class="bg-gradient-to-br from-yellow-50 to-transparent rounded-xl p-5 border border-yellow-200">
                            <label class="block text-gray-700 font-semibold mb-3 flex items-center gap-2">
                    ...
                    @if ($visit->medical_history)
                        <div class="bg-gradient-to-br from-purple-50 to-transparent rounded-xl p-5 border border-purple-200">
                            <label class="block text-gray-700 font-semibold mb-3 flex items-center gap-2">
                    ...
                    @if ($visit->diagnosis)
                        <div class="bg-gradient-to-br from-red-50 to-transparent rounded-xl p-5 border border-red-200 lg:col-span-2">
                            <label class="block text-gray-700 font-semibold mb-3 flex items-center gap-2">
        ...
        @if ($visit->secretary_description || $visit->patient_description)
            <section class="bg-white rounded-2xl shadow-lg p-6 sm:p-8 mb-8">
                <h3 class="text-xl font-bold mb-6 text-primary flex items-center gap-3 pb-4 border-b border-gray-100">
                    <svg class="w-7 h-7" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-3.582 8-8 8a8.959 8.959 0 01-2.4-.322l-3.6 1.2 1.2-3.6A8.959 8.959 0 013 12c0-4.418 3.582-8 8-8s8 3.582 8 8z"/>
                    </svg>
                ...
                <div class="bg-gradient-to-r from-yellow-50 to-orange-50 rounded-xl p-6 border border-yellow-200">
                    <div class="flex flex-col items-center gap-4">
                        <div class="flex items-center gap-1 text-3xl">
                            ...
                            @endphp
                            @for ($i = 1; $i <= 5; $i++)
                                <svg class="w-8 h-8 {{ $i <= $rating ? 'text-yellow-400' : 'text-gray-300' }}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                                    <path d="M12.0006 18.26L4.94715 22.2082L6.52248 14.2799L0.587891 8.7918L8.61493 7.84006L12.0006 0.5L15.3862 7.84006L23.4132 8.7918L17.4787 14.2799L19.054 22.2082L12.0006 18.26Z"/>
                                </svg>
                            @endfor
                        </div>
                        <p class="text-sm font-medium text-gray-700">{{ __('Rating') }}: <span class="font-bold">{{ $rating }}/5</span></p>
                    </div>
